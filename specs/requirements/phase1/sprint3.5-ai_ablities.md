...
PromptComposer 是整个 Sprint 3.5 的**基础设施中枢**，作为 ContextEngine 的子模块负责 prompt 片段加载与组合。所有 AI 调用的 system prompt 组装都通过 ContextEngine → PromptComposer 链路完成。
...
{workspace}/skills/                  # 现有 v1 格式技能（向后兼容，优先级低于 .sibylla/skills/）
│   └── {skill-name}.md
...
> **注意**：以下路径遵循现有 `src/main/services/` 统一服务层约定。详见附录 A §17A.8。

```
sibylla-desktop/src/main/services/        # 统一服务层
├── context-engine/                    # 由现有 context-engine.ts 拆分演进
│   ├── index.ts                       # ContextEngine 入口（重导出）
│   ├── PromptComposer.ts              # 新增：prompt 片段组合
│   ├── PromptLoader.ts                # 新增：prompt 文件加载
│   ├── PromptRegistry.ts              # 新增：prompt 注册与索引
│   └── templates/                     # 内部模板渲染工具
├── skill-system/                      # 由现有 skill-engine.ts 演进
│   ├── SkillRegistry.ts               # 封装原 SkillEngine，扩展 v2 格式
│   ├── SkillExecutor.ts               # 新增
│   ├── SkillLoader.ts                 # 新增
│   └── SkillValidator.ts              # 新增
├── command/                           # 扩展现有 command 目录
│   ├── command-registry.ts            # 已有，扩展 slash 解析
│   ├── types.ts                       # 已有，扩展 Command interface
│   ├── SlashCommandParser.ts          # 新增
│   └── builtin-commands/              # 已有，新增 prompt command 类型
├── sub-agent/                         # 新增
│   ├── SubAgentRegistry.ts
│   ├── SubAgentExecutor.ts
│   ├── SubAgentContext.ts
...
│   ├── HookExecutor.ts
│   └── built-in/
│       └── (委托调用现有 harness 组件，不迁移代码)
├── compact/                           # 新增
│   ├── reactiveCompact.ts
│   └── CompactOrchestrator.ts
├── harness/                           # 已有，不修改
│   ├── orchestrator.ts
│   ├── generator.ts
│   ├── evaluator.ts
...
> **架构定位**：PromptComposer 是 ContextEngine 的内部子模块，不是独立顶层模块。ContextEngine 的 `assembleContext()` 和 `assembleForHarness()` 委托给 PromptComposer 执行 prompt 片段的加载与组合。详见附录 A §17A.1。
...
```typescript
// src/main/services/context-engine/PromptComposer.ts
// 注意：PromptComposer 是 ContextEngine 的内部子模块，不是独立服务
...
> **架构定位**：Skill 系统在现有 SkillEngine 基础上渐进扩展。现有 v1 格式（扁平 .md 文件）向后兼容，新增 v2 格式（目录结构 + tools.yaml + examples）。详见附录 A §17A.2。
...
```typescript
// src/main/services/skill-system/SkillRegistry.ts
// 封装并扩展现有 SkillEngine，支持 v1（扁平 .md）和 v2（目录结构）格式
...
```typescript
// src/main/services/skill-system/SkillExecutor.ts
...
> **架构定位**：Slash Command 不建立独立系统，而是扩展现有 Command 系统（`CommandRegistry`），增加 prompt 注入能力。详见附录 A §17A.4。
...
```typescript
// src/main/services/command/SlashCommandParser.ts
...
> **架构定位**：Sub-agent 复用现有 Generator + GuardrailEngine（single 模式）+ 独立 ContextEngine 实例，不引入新的 "QueryEngine" 概念。详见附录 A §17A.6。
...
> Sub-agent 内部通过独立的 Generator + GuardrailEngine（single 模式）+ 独立 ContextEngine 实例运行，不引入 QueryEngine 概念。

```typescript
// src/main/services/sub-agent/spawnSubAgentTool.ts
...
1. **独立 Context**：独立的 Generator 实例 + AiGatewaySession，独立的 messages 数组
...
```typescript
// src/main/services/sub-agent/SubAgentExecutor.ts
...
```typescript
// src/main/services/workflow/WorkflowExecutor.ts
...
```typescript
// src/main/services/workflow/WorkflowScheduler.ts
...
> **架构定位**：Hook 系统封装现有 Harness 组件（Guardrail → PreToolUse、Guide → PreSystemPrompt、Evaluator → PostMessage、Sensor → PostToolUse），不修改现有组件代码。详见附录 A §17A.5。
...
```typescript
// src/main/services/hooks/built-in/ — 注意：内置 Hook 委托调用现有 harness 组件，不迁移代码
...
```typescript
// src/main/services/hooks/HookRegistry.ts
...
> **层级区分**：本节描述的 5 层 compact 链属于 **Context 层**（AI 对话的 messages 数组大小管理），与 Sprint 3.2 的 Memory 层压缩（MemoryCompressor、CheckpointScheduler，操作 MEMORY.md 和 memory entries）独立运行、互不干扰。详见附录 A §17A.9。
...
```typescript
// src/main/services/compact/reactiveCompact.ts
...
```typescript
// IPC — 扩展现有 ai:skill:* 命名空间
interface SkillSystemIPC {
  'ai:skill:list': (filter?: SkillFilter) => Promise<SkillV2[]>         // 已有，扩展返回类型
  'ai:skill:search': (query: string, limit?: number) => Promise<SkillV2[]> // 已有，扩展返回类型
  'ai:skill:get': (skillId: string) => Promise<SkillV2 | null>          // 新增
  'ai:skill:create': (template: SkillTemplate) => Promise<{ skillId: string; path: string }>
  'ai:skill:edit': (skillId: string) => Promise<{ path: string }>
  'ai:skill:validate': (skillId: string) => Promise<ValidationResult>
  'ai:skill:test-run': (skillId: string, testInput: string) => Promise<SkillResult>
  'ai:skill:delete': (skillId: string) => Promise<void>
  'ai:skill:export': (skillId: string) => Promise<{ bundlePath: string }>
  'ai:skill:import': (bundlePath: string, scope: SkillScope) => Promise<{ skillId: string }>
}
```
...
interface SlashCommandIPC {
  'command:search': (query: string, language?: string) => Promise<Command[]> // 已有，含 prompt commands
  'command:execute': (id: string) => Promise<void>                           // 已有
  'command:parse-slash': (input: string) => Promise<ParsedCommand | null>    // 新增
  'command:create-slash': (template: SlashCommandTemplate) => Promise<{ commandId: string }> // 新增
  'command:get-suggestions': (partial: string) => Promise<CommandSuggestion[]> // 新增
}
```
...
#### 5.4.3 EARS 验收标准
  ...
  - **UC-1**：当 spawnSubAgent 工具被调用时，系统应创建独立的 AI 调用上下文，包含独立的 messages 数组、独立的 ContextEngine 实例、独立的 GuardrailEngine 实例、独立的 Usage 记账。Sub-agent 的生成通过独立的 Generator + AiGatewaySession 实现（single 模式，不经过完整 HarnessOrchestrator）。
...
> 所有新增模型在现有 `src/shared/types.ts` 基础上扩展，不创建独立的 sprint-3.5.ts 文件。现有类型保持不变。

```typescript
// 所有模型扩展在 src/shared/types.ts 现有类型基础上
// 现有类型保持不变，新增类型向后兼容

// ---------- Skill V2（扩展现有 Skill interface）----------
...
// ---------- Command 扩展（扩展现有 Command interface）----------
// 现有 Command interface 新增可选字段：
...
| Skill（内置）| `sibylla-desktop/resources/skills/` | 目录（v2 格式）|
| Skill（工作区）| `{workspace}/.sibylla/skills/`（新路径，v2 格式）| 目录 |
| Skill（工作区旧路径）| `{workspace}/skills/`（v1 格式，向后兼容）| 扁平 .md |
...
> **注意**：以下路径已根据实际代码库校准（2026-04-23）。详见附录 A §17A.7。

- `src/main/services/mode/builtin-modes/*.ts`（Sprint 3.4）：`systemPromptPrefix` 内嵌字符串迁移到 `resources/prompts/modes/{mode}.md`，TS 文件仅保留结构化配置与 `promptFileId` 引用
- `src/main/services/harness/guardrails/engine.ts`（Sprint 3.1）：保留现有接口，HookExecutor 在 PreToolUse 节点委托调用（不封装为新文件，避免破坏 HarnessOrchestrator 的依赖注入）
- `src/main/services/harness/evaluator.ts`（Sprint 3.1）：保留现有接口，HookExecutor 在 PostMessage 节点委托调用
- `src/main/services/memory/memory-extractor.ts` + `memory-compressor.ts`（Sprint 3.2）：保留为内置服务，可独立为 sub-agent 调用入口，prompt 模板迁移到 `resources/prompts/agents/memory-curator.md`
- `src/main/services/skill-engine.ts`：渐进扩展为 `src/main/services/skill-system/` 目录，保持现有 v1 格式向后兼容

### 14.3 向后兼容策略

- Sprint 3.4 的 "mode" 字段保持不变，AiModeDefinition 新增 `promptFileId` 字段，内部从 PromptComposer 查询 system prompt
- Sprint 3.1 的 Guardrail IPC 接口保持不变（Hook 系统在内部委托调用，不暴露新接口）
- Sprint 3.1 的 Guide/Sensor 接口保持不变
- 现有 `Skill` type 保持不变，新增 `SkillV2` 扩展类型
- 现有 IPC 通道 `ai:skill:*` 保持，扩展返回类型
- 现有 Command 系统保持，扩展 `Command` interface 支持 Slash Command
- 所有新能力默认关闭（feature flag），用户主动启用
- 现有 `AssembledContext` interface 保持，新增可选 `promptParts` 字段
- `ContextLayerType` 从 4 值扩展为 9 值，不删除已有值
...
## 第十七章 现有代码库冲突适配方案（附录 A）

> 本章节记录 Sprint 3.5 规格与当前代码库（截至 2026-04-23）之间的冲突分析结果及调整方案。
> 核心原则：**新需求优先，最小侵入，渐进演进，不破坏已稳定功能**。

### 17A.1 PromptComposer 与 ContextEngine 的关系（对应冲突 C2）

**结论**：PromptComposer 不作为独立顶层模块，而是作为 ContextEngine 的内部子模块。

**架构调整**：
- PromptComposer 位于 `src/main/services/context-engine/PromptComposer.ts`
- ContextEngine 的 `assembleContext()` 和 `assembleForHarness()` 内部委托给 PromptComposer
- 现有 `AssembledContext` 接口保留，新增可选字段 `promptParts?: PromptPart[]`（溯源用）
- 现有 `ContextLayerType` 从 `'always' | 'manual' | 'skill' | 'memory'` 扩展为 `'always' | 'manual' | 'skill' | 'memory' | 'mode' | 'tool' | 'agent' | 'hook' | 'context'`
- 硬编码 `SYSTEM_PROMPT_BASE` 替换为从 `resources/prompts/core/identity.md` 加载

**涉及的现有文件变更**：
- `src/main/services/context-engine.ts` → 拆分为 `src/main/services/context-engine/index.ts` + `PromptComposer.ts` + `PromptLoader.ts` + `PromptRegistry.ts`
- `src/shared/types.ts` → `AssembledContext` 新增 `promptParts` 字段，`ContextLayerType` 扩展

### 17A.2 Skill 系统渐进演进（对应冲突 C1、C6、C13）

**结论**：在现有 SkillEngine 基础上渐进扩展，不推翻重建。

**架构调整**：
- 现有 `Skill` type 扩展为 `SkillV2`（继承原字段，新增 version/author/category/tags/scope/source/triggers/allowedTools）
- 现有扁平 `.md` 格式作为 v1 格式继续支持（向后兼容）
- 新增目录格式（`_index.md` + `prompt.md` + `tools.yaml` + `examples/`）作为 v2 格式
- `SkillEngine` 扩展为 `SkillRegistry`（封装原 SkillEngine），新增 v2 格式解析
- Skill 搜索路径：
  1. `{workspace}/.sibylla/skills/`（新路径，v2 格式）
  2. `{workspace}/skills/`（现有路径，v1 格式，标记 legacy）
  3. `resources/skills/`（内置，v2 格式）
- 现有 IPC 通道 `ai:skill:list`、`ai:skill:search` 保持，返回类型扩展为 `SkillV2[]`
- 新增 IPC 通道在 `ai:skill:` 命名空间下：`ai:skill:validate`、`ai:skill:create`、`ai:skill:edit`、`ai:skill:delete`、`ai:skill:export`、`ai:skill:import`

**涉及的现有文件变更**：
- `src/main/services/skill-engine.ts` → 扩展为 `src/main/services/skill-system/` 目录（SkillRegistry.ts、SkillLoader.ts、SkillExecutor.ts、SkillValidator.ts）
- `src/shared/types.ts` → 新增 `SkillV2`、`SkillTrigger`、`SkillToolsConfig` 类型
- `src/main/ipc/handlers/ai.handler.ts` → 扩展 skill 相关 IPC

### 17A.3 Mode Prompt 混合迁移（对应冲突 C3）

**结论**：`systemPromptPrefix` 迁移为文件，其余结构化配置保留在 TypeScript。

**架构调整**：
- 5 个内置 mode 的 `systemPromptPrefix` 从 `builtin-modes/*.ts` 提取到 `resources/prompts/modes/{mode}.md`
- `AiModeDefinition` interface 新增 `promptFileId?: string` 字段（如 `modes.plan`）
- `AiModeRegistry.buildSystemPromptPrefix()` 改为从 PromptComposer 加载 prompt 文件
- `outputConstraints`、`modeEvaluatorConfig`、`uiHints`、`requiresContext` 等保留在 TS 定义中
- Mode TS 文件不再内嵌 prompt 字符串，仅保留结构化配置 + prompt 文件引用

**涉及的现有文件变更**：
- `src/main/services/mode/builtin-modes/*.ts` → 删除内嵌 prompt 字符串，改为 `promptFileId` 引用
- `src/main/services/mode/ai-mode-registry.ts` → `buildSystemPromptPrefix()` 委托给 PromptComposer
- `src/main/services/mode/types.ts` → `AiModeDefinition` 新增 `promptFileId` 字段

### 17A.4 Slash Command 作为 Command 系统扩展（对应冲突 C4）

**结论**：不建立独立的 SlashCommand 系统，扩展现有 Command 系统支持 prompt 注入。

**架构调整**：
- `Command` interface 新增可选字段：
  ```typescript
  interface Command {
    // ... 现有字段 ...
    promptTemplate?: string      // Slash Command 的 prompt 模板
    params?: CommandParam[]      // 参数定义
    isSlashCommand?: boolean     // 标识是否为 /xxx 触发
  }
  ```
- `CommandRegistry` 新增 `resolveBySlash(prefix: string)` 方法
- Slash Command 的 `/xxx` 输入在 AIHandler 层拦截，走 CommandRegistry.resolveBySlash()
- 已有的 `SlashCommandMenu.tsx`（editor 中的富文本命令菜单）不受影响
- `/help` 命令动态合并 action commands 和 prompt commands

**涉及的现有文件变更**：
- `src/main/services/command/types.ts` → 扩展 `Command` interface
- `src/main/services/command/command-registry.ts` → 新增 slash 解析方法
- `src/main/ipc/handlers/command.ts` → 新增 slash command 相关 IPC

### 17A.5 Hook 系统与 Harness 的映射（对应冲突 C5）

**结论**：Hook 节点封装现有 Harness 组件，不破坏现有接口。

**架构调整**：

| Hook 节点 | 映射到现有组件 | 说明 |
|---|---|---|
| PreUserMessage | IntentClassifier | 意图分析（已存在于 orchestrator 流程） |
| PreSystemPrompt | GuideRegistry | 指南注入（已存在于 assembleForHarness） |
| PreToolUse | GuardrailEngine | 护栏检查（现有 guardrails/engine.ts） |
| PostToolUse | SensorFeedbackLoop | 传感器反馈（已存在） |
| PostMessage | Evaluator | 质量评估（现有 evaluator.ts） |
| PreCompaction | — | 新增，控制压缩行为 |
| PostCompaction | — | 新增，压缩后处理 |
| StopCheck | — | 新增，模型停止决策 |

- GuardrailEngine 保持现有接口，HookExecutor 在 PreToolUse 节点委托调用
- Evaluator 保持现有接口，HookExecutor 在 PostMessage 节点委托调用
- 新增的 3 个节点（PreCompaction/PostCompaction/StopCheck）为纯增量
- 用户 Hook 支持 `PreToolUse`、`PostToolUse`、`PreCompaction`、`PostCompaction` 四个节点
- 用户 Hook 只允许返回 `warn`，不允许 `block`（block 权限仅限内置 Hook）

**涉及的现有文件变更**：
- `src/main/services/harness/orchestrator.ts` → 在关键流程点调用 HookExecutor
- 新增 `src/main/services/hooks/HookRegistry.ts`、`HookExecutor.ts`
- GuardrailEngine、Evaluator 等文件不修改

### 17A.6 Sub-agent 运行时抽象（对应冲突 C9）

**结论**：Sub-agent 复用 HarnessOrchestrator（single 模式）+ 独立 ContextEngine 实例。

**架构调整**：
- 不引入 "QueryEngine" 概念
- `SubAgentExecutor` 内部创建：
  1. 独立的 `ContextEngine` 实例（加载 sub-agent prompt，不继承主 agent 对话历史）
  2. 独立的 `Generator` 实例（通过 `AiGatewaySession`，独立 session）
  3. 可选的 `GuardrailEngine` 实例（权限边界检查）
- Sub-agent loop = Generator + GuardrailEngine 的简化循环（非完整 HarnessOrchestrator）
- 主循环通过 `spawnSubAgent` 工具触发，等待结构化结果返回

**涉及的现有文件变更**：
- 新增 `src/main/services/sub-agent/` 目录
- `src/main/services/harness/generator.ts` → 抽取可复用的生成逻辑

### 17A.7 迁移源文件路径修正（对应冲突 C10）

**结论**：修正 §14.2 中的迁移源文件路径。

**修正映射**：

| Spec 原引用 | 实际路径 | 说明 |
|---|---|---|
| `src/main/ai/modePrompts.ts` | `src/main/services/mode/builtin-modes/*.ts`（5个文件） | mode prompt 内嵌在 TS 中 |
| `src/main/ai/guardrail.ts` | `src/main/services/harness/guardrails/engine.ts` | GuardrailEngine |
| `src/main/ai/evaluator.ts` | `src/main/services/harness/evaluator.ts` | Evaluator |
| `src/main/ai/memoryCurator.ts` | `src/main/services/memory/memory-extractor.ts` + `memory-compressor.ts` | 分散在 memory 模块 |

### 17A.8 代码目录对齐（对应冲突 C11）

**结论**：新模块放入 `src/main/services/` 下，保持统一的 services 层。

**修正映射**：

| Spec 规划目录 | 调整为 |
|---|---|
| `src/main/prompt-library/` | `src/main/services/context-engine/`（ContextEngine 子模块） |
| `src/main/skill-system/` | `src/main/services/skill-system/`（替换现有 skill-engine.ts） |
| `src/main/slash-command/` | `src/main/services/command/`（扩展现有 command 目录） |
| `src/main/sub-agent/` | `src/main/services/sub-agent/`（新增） |
| `src/main/workflow/` | `src/main/services/workflow/`（新增） |
| `src/main/hooks/` | `src/main/services/hooks/`（新增） |
| `src/main/compact/` | `src/main/services/compact/`（新增） |

### 17A.9 Compact 链分层（对应冲突 C12）

**结论**：明确区分 Memory 层压缩与 Context 层压缩。

**分层定义**：
- **Memory 层**（Sprint 3.2 已有）：MemoryCompressor（精选记忆压缩）、CheckpointScheduler（定期检查点）。操作 MEMORY.md 和 memory entries，与 AI 对话窗口大小无关。
- **Context 层**（本 Sprint）：5 层 compact 链管理 AI 对话的 messages 数组大小。属于 ContextEngine 的上下文窗口管理。
  - snipCompact：剪掉单条过大工具结果
  - microcompact：局部段落压缩
  - contextCollapse：分散段落折叠为摘要
  - autoCompact：主动汇总整段历史
  - reactiveCompact：被动兜底（API 413 等）
- 两者独立运行，互不干扰。
