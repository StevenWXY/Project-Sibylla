...
**团队记忆系统** — 三层分层存储架构（原始日志 + 精选记忆 + 归档），自动积累团队知识。心跳检查点、预压缩冲洗、决策日志、自愈优化等动态维护机制，确保 AI 始终拥有完整的团队上下文和历史决策。
...
Electron 桌面客户端（编辑 / AI 对话 / 本地存储 / Git 操作 / 记忆管理）  
↕  
HTTPS / WSS  
↕  
Sibylla 云端服务（认证 / Git 托管 / 语义搜索 / AI 网关 / 积分账本 / 记忆同步）
